...
m1_predicted_values <- m1_prediction[[1]][c("condition", "time_norm", "fit", "ul", "ll")] |>
...
sum_pupil_df |>
  ...
  aes(x = time_norm,
      ...
      fill = speaker) +
  geom_line() +
  geom_line(data = m1_predicted_values, aes(x = time_norm, y = fit, color = speaker, fill = speaker), linetype = 2) +
...
Adding scaled t distribution to fix model fit. The model was originally running into an error with the random smooth for phrase. Therefore, it was removed in model 2.
...
sum_pupil_df |>
  ...
  aes(x = time_norm,
      ...
      fill = speaker) +
  geom_line() +
  geom_line(data = m2_predicted_values, aes(x = time_norm, y = fit, color = speaker, fill = speaker), linetype = 2) +
  geom_vline(xintercept = 0, size = 0.35) +
  geom_vline(xintercept = 1910, linetype = 2) +
  annotate("rect", xmin = 500, xmax = 2910, ymin = 0, ymax = 200, alpha = 0.2) +
  coord_cartesian(ylim = c(-10, 250), xlim = c(0, 4910)) +
...
Adjusting random smooth to be for each listener by condition
...
m3_predicted_values <- m3_prediction[[1]][c("condition", "time_norm", "fit", "ul", "ll")] |>
  dplyr::mutate(rep_acc = ifelse(grepl("inaccurate", condition, ignore.case = T), "inaccurate", "accurate"),
                speaker = ifelse(grepl("Control", condition, ignore.case = T), "Control", "ALS"))
...
sum_pupil_df |>
  ...
  aes(x = time_norm,
      ...
      fill = rep_acc) +
  geom_line() +
  geom_line(data = m3_predicted_values, aes(x = time_norm, y = fit, color = rep_acc, fill = rep_acc), linetype = 2) +
  geom_vline(xintercept = 0, size = 0.35) +
  geom_vline(xintercept = 1910, linetype = 2) +
  annotate("rect", xmin = 500, xmax = 2910, ymin = 0, ymax = 200, alpha = 0.2) +
  coord_cartesian(ylim = c(-10, 250), xlim = c(0, 4910)) +
...
compareML(m2_pupil, m3_pupil, signif.stars=T)
