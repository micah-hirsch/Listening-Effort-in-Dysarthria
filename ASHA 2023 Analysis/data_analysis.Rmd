...
Since the trials for the ALS speaker are much longer than the control speaker's trials, we are going to conduct an alternative analysis. First, we are going to resample the ALS speaker's trials so the length of the trials are comparable to the control speaker's trials. Then, we will conduct the mixed-effects models on the resampled data.

```{r}

# Rescaling the ALS speaker's trials to be more consistent with the Control speaker.
...
ALS.binned <- data %>%
  dplyr::filter(speaker == "ALS") %>%
  mutate(bins = floor(timebins/bin.width)*bin.width) %>%
  dplyr::group_by(subject, trial_c, speaker, bins, effort_rating,
                  code, counterbalance) %>%
  dplyr::summarize(pupil_dilation = mean(pupil.binned)) %>%
  dplyr::ungroup()
