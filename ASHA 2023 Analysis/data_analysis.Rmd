...
rm(m0, m1, m2, m3, m4, pupil_plot, pupil_roi, data, control_bobyqa, my_pal)
...
# Ran into convergence issues, so added the nlminb optimizer to the model.
# Model had simgular fit issues, so removed the (trial_c|code) random effect.

library(optimx)

m1 <- lmer(peak_pupil ~ 1 + (trial_c|subject), 
...
m2 <- lmer(peak_pupil ~ trial_c + (trial_c|subject), 
           data = peak_pupil_df, control = lmerControl(
                           optimizer ='optimx', optCtrl=list(method='nlminb')))
...
m3 <- lmer(peak_pupil ~ trial_c + speaker + (trial_c|subject) , 
              data = peak_pupil_df, control = lmerControl(
                           optimizer ='optimx', optCtrl=list(method='nlminb')))
...
m4 <- lmer(peak_pupil ~ trial_c + speaker + trial_c*speaker + (trial_c|subject) , 
              data = peak_pupil_df, control = lmerControl(
                           optimizer ='optimx', optCtrl=list(method='nlminb')))
