...
Both the conditional and unconditional ICC is 0.099.
...
To control for trial order, trial_c is added as a fixed effect to model 2. A likelihood ratio test was also conducted to test whether model 2 provides a better fit to the data compared to model 1
...
As indicated from the model, adding trial order did not significantly improve the model fit to the data (chi-sq(1) = 0.412, p = 0.52). This indicates that trial order is not an important predictor for perceived listening effort ratings and that ratings for the speakers were fairly consistent across trials.
...
Based on the model output, adding speaker to the model improved the model fit to the data (chi-sq(1) = 469.74, p <0.001). The fixed effect estimate indicates that, while controlling for trial order, the perceived listening effort rating across listeners and phrases is about 2.35 higher for ALS speaker.
...
Based on the model output, adding the interaction term did not significantly improve model fit (chi-sq(1) = 0.14, p= 0.70). The fixed effect estimate for the interaction term is also not significant. This indicates that effort ratings across the trials for the ALS speaker were not significantly different from each other.
...
# This code block is a Work in Progress. Since the phrase legnth between the ALS speaker and the Control speaker are very different, this block is attempting to downsample the ALS trials to be a similar length of the control trials. The current code is able to downsample to a point where the number of rows for ALS trials and control trials are similar (in the new normed_pupil df). However, the timestamp labels were not updated yet. This analysis may be moved to an alternative script to further flesh this out.

# This code block is not included in the ASHA 2023 output file.
...
data1 <- data %>%
  filter(speaker == "ALS") %>%
  group_by(code) %>%
  mutate(bins = round(timebins/bin.length)*bin.length) %>%
  group_by(subject, trial, bins, speaker, code) %>%
  dplyr::summarize(pupil = mean(pupil.binned)) %>%
  ungroup()

control <- data %>%
  filter(speaker == "Control") %>% 
  select(subject, trial, timebins, speaker, code, pupil.binned) %>%
  rename(pupil = pupil.binned,
         bins = timebins)

normed_pupil %>%
  count(speaker, code) %>%
  group_by(speaker) %>%
  summarize(row_m = mean(n), sd = sd(n))
...
The adjusted ICC for the unconditional growth model is .109. The unadjusted ICC for the conditional growth model is .107.
...
In model two, the interaction between poly1 and speaker was added to the model as a fixed effect to test whether linear growth was greater for the ALS speaker compared to the control speaker. A likelihood ratio test was also conducted to test if model 2 provides a better fit to the data compared to model 1.
...
Based on the model output, the interaction between poly1 and speaker is significant, indicating that, across listeners, linear growth in pupil dilation while listening to the ALS speaker was significantly greater than when listeners were listening to the control speaker. The likelihood ratio test also indicated that model 2 provided a better fit to the data than model 1 (chi-sq(1) = 649.22, p <0.001).
...
```
Based on the model output, the interaction between the quadratic growth term and speaker improved the model fit (ch-sq(1) = 147.72, p<0.001). This indicates, on average, the shape of the listeners' pupil dilation is different while listening to the ALS speaker vs the control speaker.
...
```
Based on the model output, model 4 provides a better fit to the data compared to model 3 (chi-sq(1) = 9.25, P = 0.002). What is interesting is that the only fixed effects that are significant in this model are the main effect of the cubic growth term and the interaction between the cubic growth term and speaker, indicating the cubic pupil dilation growth is significantly different between these two speakers.

```{r, message = F, warning = F}

sjPlot::tab_model(m4, file = "Final Model Outputs/final_pupil_dilation_model.html")
...
sjPlot::tab_model(m1, m2, m3, m4, file = "Final Model Outputs/pupil_dilation_model.html")
...
Model 1 provided a better fit to the data (chi-sq(2) = 43.44, p < 0.001).
...
Adding trial to the model as a fixed effect significantly improved model fit (chi-sq(1) = p<0.001). Based on the fixed effect estimate for trial order, peak pupil dilation decreased approximately -1.23 units on average with each trial.
...
Adding speaker to the model improved the model fit to the data (ch-sq(1), p <0.001). Controlling for trial order, listeners' peak pupil dilation, on average, was about 85.22 units higher compared to when they were listening to the control speaker. This indicates listeners were exerting more effort while listening to the ALS speaker compared to the control speaker.
...
final_model <- lmer(peak_pupil ~ trial_c + speaker + (trial_c|subject) , 
              data = peak_pupil_df, control = lmerControl(
                           optimizer ='optimx', optCtrl=list(method='nlminb')))

sjPlot::tab_model(final_model, file = "Final Model Outputs/peak_pupil_model.html")
...
rm(data, m0, m1, m2, m3, m4, peak_pupil_des, peak_pupil_df, pupil_roi, my_pal)
